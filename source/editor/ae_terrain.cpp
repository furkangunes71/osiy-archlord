...
struct region_tool {
	...
	struct ap_map_region_template * replace_target;
};
...
struct ae_terrain_module {
	...
	bgfx_texture_handle_t region_icon;
	bgfx_texture_handle_t tile_icon;
};
	...
	struct ac_mesh_vertex ** triangle2)
{
	...
	if (count + 2 > paint->max_triangle_count) {
		for (i = 0; i < COUNT_OF(materials); i++)
			ac_mesh_release_material(mod->ac_mesh, &materials[i]);
		return FALSE;
	}
	...
	/* Set pointers AFTER all pushes to avoid invalidation
	 * from vector reallocation. */
	*(triangles[0]) = &paint->vertices[vec_count(paint->vertices) - 6];
	*(triangles[1]) = &paint->vertices[vec_count(paint->vertices) - 3];
	for (i = 0; i < COUNT_OF(materials); i++)
...
boolean oninitialize(struct ae_terrain_module * mod)
{
	...
	if (!BGFX_HANDLE_IS_VALID(mod->paint.tex_alpha)) {
		WARN("Failed to load alpha texture, paint tool may not work correctly.");
	}
	...
	mod->region_icon = ac_texture_load(mod->ac_texture,
		"content/textures/region-edit.png", NULL);
	if (!BGFX_HANDLE_IS_VALID(mod->region_icon)) {
		ERROR("Failed to load region edit tool icon.");
		return FALSE;
	}
	mod->tile_icon = ac_texture_load(mod->ac_texture,
		"content/textures/tile-edit.png", NULL);
	if (!BGFX_HANDLE_IS_VALID(mod->tile_icon)) {
		ERROR("Failed to load tile edit tool icon.");
		return FALSE;
	}
...
void ae_terrain_toolbar(struct ae_terrain_module * mod)
{
	...
	case EDIT_MODE_REGION: {
		...
		ImGui::SameLine();
		{
			char rlabel[256] = "Not Selected";
			if (tool->replace_target) {
				snprintf(rlabel, sizeof(rlabel), "[%u] %s",
					tool->replace_target->id,
					tool->replace_target->glossary);
			}
			ImGui::SetNextItemWidth(160.f);
			if (ImGui::BeginCombo("Replace with", rlabel)) {
				uint32_t ri;
				ImGui::Text("Visible Regions");
				ImGui::Separator();
				for (ri = 0; ri < COUNT_OF(tool->colors); ri++) {
					if (tool->colors[ri].refcount) {
						struct ap_map_region_template * rt =
							ap_map_get_region_template(mod->ap_map, ri);
						if (rt) {
							snprintf(rlabel, sizeof(rlabel),
								"[%03u] %s##R", ri, rt->glossary);
						}
						else {
							snprintf(rlabel, sizeof(rlabel),
								"[%03u] Invalid Region##R", ri);
						}
						if (ImGui::Selectable(rlabel,
								rt && rt == tool->replace_target) && rt) {
							tool->replace_target = rt;
						}
					}
				}
				ImGui::Dummy(ImVec2(0.0f, 10.0f));
			...
			ImGui::SameLine();
			if (tool->selected && tool->replace_target &&
					tool->selected != tool->replace_target) {
				if (ImGui::Button("Replace")) {
					ac_terrain_replace_region_id(
						mod->ac_terrain,
						tool->selected->id,
						tool->replace_target->id);
				}
